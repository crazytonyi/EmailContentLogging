...
class SAMLController
{
            ...
            if ($token->isAuthenticated()) {
                ...
                if (is_null($response)) {
                    $urlQuery = [
                        ...
                        'IdPSessionIndex' => $rememberMeToken->getAttribute('IdPSessionIndex'),
                    ];
    ...
    public function logoutInitAction(Application $app, Request $request)
    {
        try {
            ...
            $app->getCookieService()->setLoggedInIdentitiesCookie($logoutResponse);
    ...
    public function logoutAction(Application $app, Request $request)
    {
        ...
        try {
            ...
            $app->getLogger()->error($e->getMessage(), ['event' => 'logout_failed', 'tags' => ['IdM.saml']]);
        ...
        if (!$resultToken->isAuthenticated()) {
            ...
            $nameId = key_exists('nameId', $parameters) ? $parameters['nameId'] : '';
            $userSRN = (string)$this->getUserSRN($app->getRememberMeService(), $nameId);
            if (!empty($url)) {
                $url = $this->extendUrl($url, $parameters);
            }

            $response = $app->getLogoutService()->logout($request, $userSRN, $url);
            $app->getLogger()->info('Logout for user {user_name} with SRN {user_srn}', [
                'user_name' =>  $nameId !== '' ? $nameId : 'unknown',
                'user_srn' => $userSRN !== '' ? $userSRN : 'unknown',
                'tags' => ['IdM.saml'],
                'event' => 'after_logout',
            ]);
            return $response;
     ...
     * @return string|null
     */
    private function getUserSRN(RememberMe $rememberMe, string $nameId): ?string
    {
        /** @var $token RememberMeToken $token */
        foreach ($rememberMe->list() as $token) {
            if (
                $token->getUsername() === $nameId
                && $token->getProviderKey() === Providers::PROVIDER_KEY_SAML
            ) {
                return $token->getSRN();
            }
        }
