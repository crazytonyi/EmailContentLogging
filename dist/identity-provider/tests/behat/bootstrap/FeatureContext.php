...
class FeatureContext extends BehatFormatterContext implements PageObjectAware
{
    ...
    /**
     * @Then I skip cookie consent
     * @And I skip cookie consent
     */
    public function iSkipCookieConsent()
    {
        $this->iWaitUntilTheLoadingIsCompleted();
        $cookieConsentElem = $this->getSession()->getPage()->waitFor(
            5,
            function () {
                return $this->getSession()->getPage()->find('css', '[data-name=cookie_consent] input');
            }
        );
        if (!$cookieConsentElem) {
            return;
        }
        $cookieConsentElem->click();

        $this->getSession()->getPage()->find('css', 'a.btn-primary[name=continue_button]')->click();
    }
