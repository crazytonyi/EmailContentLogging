...
class ConsentControllerTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @var RememberMeToken | \PHPUnit_Framework_MockObject_MockObject
     */
    ...
    protected function setUp()
    {
        ...
        $this->userToken->method('getUser')->willReturn($user);

        $this->rememberMeToken = $this->createMock(RememberMeToken::class);
        $this->rememberMeToken->method('getUser')->willReturn($user);
        $this->rememberMeToken->method('getSource')->willReturn($this->userToken);
    ...
    public function testConsentInitAction(): void
    {
        ...
        $this->urlGenerator->expects($this->once())
            ...
            ->with(
                'loginRender',
                [
                    'tenant_hint' => $tenantSrn,
                    'login_hint' => $username,
                ]
            )->willReturn('http://loginRender?With=tenant_hint');
        $this->application->expects($this->once())->method('redirect')->with('http://loginRender?With=tenant_hint');
        $this->consentController->consentInitAction($this->application, $this->request);
    }

    /**
     * Provides data for testConsentInitActionWithGetLanguages
     *
     * @return array
     */
    public function consentInitActionWithGetLanguagesProvider(): array
    {
        return [
            'crmConsent' => [
                'tenantId' => '1000000001',
                'clientId' => 'srn:cloud:iam:eu:1000000001:app:crm:123',
                'clientUrl' => 'http://mango.url',
                'expectedUrl' => 'http://mango.url',
            ],
            'nativeConsent' => [
                'tenantId' => '1000000001',
                'clientId' => 'srn:dev:iam:::app:native:123',
                'clientUrl' => 'http://localhost.url',
                'expectedUrl' => '',
            ],
            'crmConsentWithoutUrl' => [
                'tenantId' => '1000000001',
                'clientId' => 'srn:cloud:iam:eu:1000000001:app:crm:123',
                'clientUrl' => '',
                'expectedUrl' => '',
     ...
     *
     * @covers ::consentInitAction
     * @dataProvider consentInitActionWithGetLanguagesProvider
     */
    public function testConsentInitActionWithGetLanguages(
        string $tenantId,
        string $clientId,
        string $clientUrl,
        string $expectedUrl
        ...
        $this->consentToken->method('getClientId')->willReturn($clientId);
        $this->consentToken->method('getClientUrl')->willReturn($clientUrl);

        $this->metadataService->expects($this->once())
            ->method('getLanguagesFromSource')
            ->with(Srn\Converter::toString($srn), $expectedUrl);
    ...
    public function testConsentFinishActionConsentScopesDoNotMatch()
    {
        ...
        $this->rememberMeService->method('retrieve')->willReturn($this->rememberMeToken);
    ...
    public function testConsentFinishActionRestFlowAcceptRequest()
    {
        ...
        $this->oAuth2Service->expects($this->once())
            ...
            ->with($this->consentToken, $this->rememberMeToken);
    ...
    public function testConsentConfirmationActionAuthAutoAcceptConsent(string $clientId): void
    {
        ...
        $this->oAuth2Service
            ...
            ->with(
                ...
                $this->equalTo($this->rememberMeToken)
            );
        $this->application
            ->expects($this->once())
            ->method('redirect')
