...
var submitUserSelectionForm = function (e) {
    var form = document.forms['login'];
    if (!e && !e.currentTarget && !form) {
        console.error('Element not found. Can not proceed with form submission', e, form);
        return;
    }
    form['tid'].value = e.currentTarget.getAttribute('data-tid');
    form['user_srn'].value = e.currentTarget.getAttribute('data-user-srn');
    form['user_name'].value = e.currentTarget.getAttribute('data-userName');
...
var onDOMContentLoaded = function (e) {
    var loginButton = document.getElementById('submit_btn'),
        ...
        userSessionsButtons = document.querySelectorAll('#user-selection-list > li');
    ...
    if (userSessionsButtons) {
      for (var i = 0; i < userSessionsButtons.length; i++) {
        userSessionsButtons[i].onclick = submitUserSelectionForm;
      }
    }
