...
({
    ...
    securityPolicyViolationHandler: function(event) {
        // The event might return a blockedURI with a slash at the end in some browsers.
        // To ensure that in fact the same address is checked we remove the last slash.
        var currentURI = this.removeLastSlashFromURI(this.value);
        var blockedURI = this.removeLastSlashFromURI(event.blockedURI);
        if (currentURI === blockedURI) {
    ...
    getCSPMessage: function() {
        if (app.user.get('type').toLowerCase() === 'admin') {
            return app.lang.get('LBL_CSP_ERROR_MESSAGE_ADMIN');
        } else {
            return app.lang.get('LBL_CSP_ERROR_MESSAGE_USER');
        }
    },
