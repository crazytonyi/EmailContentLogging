...
$mod_strings = array(
    ...
    'ERR_CHECKSYS_FASTCGI'              => 'Забелязваме, че не използвате FastCGI handler mapping за PHP. Ще трябва да инсталирате/конфигурирате версия, която е съвместима с приложението Sugar.  Моля, направете справка в матрицата за съвместимост в бележките за изданието на поддържаните версии. Моля, вижте <a href="http://www.iis.net/php/" target="_blank" rel="nofollow noopener noreferrer">http://www.iis.net/php/</a> за подробности ',
    ...
    'LBL_CHECKSYS_DB_SUPPORT_NOT_AVAILABLE' => 'Не е намерена „Поддръжка на база данни". Уверете се, че имате необходимите драйвери за един от следните поддържани типове бази данни: MySQL, MS SQLServer, Oracle или DB2. Може да се наложи да премахнете коментара на разширението във файла php.ini или да компилирате отново с правилния двоичен файл според вашата версия на PHP. Направете справка с вашето ръководство за PHP за допълнителна информация за начина, по който да активирате „Поддръжка на база данни".',
...
<br> Please correct your <a target=_new href='http://us2.php.net/manual/en/ref.session.php'>PHP configuration</a> in the php.ini file located here below.",
    ...
    'meeting_notification_email' => array(
        'name' => 'Имейли с уведомления за срещи',
        'subject' => 'Среща от SugarCRM - $event_name ',
        'description' => 'Шаблонът се използва, когато системата изпраща уведомления за среща до потребител.',
        'body' => '<div>
	<p>До: $assigned_user</p>

	<p>$assigned_by_user ви кани на среща</p>

	<p>Тема: $event_name<br/>
	Начална дата: $start_date<br/>
	Крайна дата: $end_date</p>

	<p>Описание: $description</p>

	<p>Приемате срещата:<br/>
	<<a href="$accept_link">$accept_link</a>></p>
	<p>Приемате срещата с колебания:<br/>
	<<a href="$tentative_link">$tentative_link</a>></p>
	<p>Отказвате срещата:<br/>
	<<a href="$decline_link">$decline_link</a>></p>
</div>',
        'txt_body' =>
            'До: $assigned_user

$assigned_by_user ви покани на среща

Тема: $event_name
Начална дата: $start_date
Крайна дата: $end_date
...
<$decline_link>',
    ...
    'call_notification_email' => array(
        'name' => 'Имейли с уведомления за повикване',
        'subject' => 'Повикване от SugarCRM - $event_name ',
        'description' => 'Шаблонът се използва, когато системата изпраща уведомления за повикване до потребител.',
        'body' => '<div>
	<p>До: $assigned_user</p>

	<p>$assigned_by_user ви кани на разговор</p>

	<p>Тема: $event_name<br/>
	Начална дата: $start_date<br/>
	Продължителност: $hoursh, $minutesm</p>

	<p>Описание: $description</p>

	<p>Приемате повикването:<br/>
	<<a href="$accept_link">$accept_link</a>></p>
	<p>Приемате повикването с колебания:<br/>
	<<a href="$tentative_link">$tentative_link</a>></p>
	<p>Отказвате повикването:<br/>
	<<a href="$decline_link">$decline_link</a>></p>
</div>',
        'txt_body' =>
            'До: $assigned_user

	$assigned_by_user ви кани на разговор

	Тема: $event_name
	Начална дата: $start_date
	Продължителност: $hoursh, $minutesm

	Описание: $description

	Приемате повикването:
	<$accept_link>

	Приемате повикването с колебание
	<$tentative_link>

	Отказвате повикването:
        <$decline_link>',
    ),
...
Прикаченият файл съдържа автоматично генерирана справка, която е насрочена за Вас.

Име на справка: $report_name

Дата и час на изпълнение на справката: $report_time',
    ),

    'comment_log_mention_email' => [
        'name' => 'Уведомление с имейл за системен регистър с коментари',
        'subject' => 'SugarCRM - $initiator_full_name Ви спомена в $singular_module_name',
        'description' => 'Шаблонът се използва, за да се изпрати уведомление по имейл на потребители, които са отбелязани в раздел на регистър с коментари.',
        'body' =>
            '<div>
                <p>Вие сте споменати в следния регистър с коментари на запис:  <a href="$record_url">$record_name</a></p>
                <p>Влезте в Sugar, за да видите коментара.</p>
            </div>',
        'txt_body' =>
'Вие сте споменати в следния регистър с коментари на запис:  $record_name
...
'portal_forgot_password_email_link' => [
    'name' => 'Поща за забравена парола за портал',
    'subject' => 'Нулирайте паролата за вашия акаунт',
    'description' => 'Този шаблон се използва за изпращане на връзка към потребител, която той да щракне, за да промени паролата за акаунт на потребител на портала.',
    'body' => '<div><table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width="550" align=\"\&quot;\&quot;center\&quot;\&quot;\"><tbody><tr><td colspan=\"2\"><p>Наскоро заявихте промяна на паролата на вашия акаунт. </p><p>Щракнете върху връзката отдолу, за да промените паролата си:</p><p> <a href="$portal_user_link_guid">$portal_user_link_guid</a> </p> </td> </tr><tr><td colspan=\"2\"></td> </tr> </tbody></table> </div>',
    'txt_body' =>
'
    Наскоро заявихте промяна на паролата на своя акаунт.

    Щракнете върху връзката отдолу за да промените паролата си:

    $portal_user_link_guid',
],

    'portal_password_reset_confirmation_email' => [
        'name' => 'Електронна поща за потвърждение за промяна на парола за портала',
        'subject' => 'Паролата на вашия акаунт е променена',
        'description' => 'Този шаблон се използва, за да се изпрати потвърждение на потребител на портала, че паролата за акаунта му е променена.',
        'body' => '<div><table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width="550" align=\"\&quot;\&quot;center\&quot;\&quot;\"><tbody><tr><td colspan=\"2\"><p>Тази електронна поща е за потвърждение, че паролата за акаунта ви са портала е променена. </p><p>Използвайте връзката отдолу, за да влезете в портала:</p><p> <a href="$portal_login_url">$portal_login_url</a> </p> </td> </tr><tr><td colspan=\"2\"></td> </tr> </tbody></table> </div>',
        'txt_body' =>
            '
    Тази електронна поща е за потвърждение, че паролата за акаунта ви за портала е променена.

    Използвайте връзката отдолу, за да влезете в портала:
