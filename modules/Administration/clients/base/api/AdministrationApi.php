...
class AdministrationApi extends SugarApi
{
     ...
     */
    public function setCSPConfig(ServiceBase $api, array $args)
    {
        $this->ensureAdminUser();
        $admin = BeanFactory::getBean('Administration');
        ...
        foreach ($args as $key => $value) {
            if (substr($key, 0, 4) === $prefix) {
                if ($key === 'csp_default_src') {
                    $sourceValues = array_filter(explode(' ', sanitizeCspDirectiveValue($value)));
                    foreach ($sourceValues as $sourceValue) {
                        if (!isValidCSPSrcValue($sourceValue)) {
                            throw new SugarApiExceptionInvalidParameter($key . 'is not valid');
                        }
                    }
                }
                if ($admin->saveSetting($category, str_replace($prefix, '', $key), $value, $api->platform)) {
