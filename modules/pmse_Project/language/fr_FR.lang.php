<?php
/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */


$mod_strings = array (
  'LBL_MODULE_NAME' => 'Définitions des processus',
  'LBL_MODULE_TITLE' => 'Définitions des processus',
  'LBL_MODULE_NAME_SINGULAR' => 'Définition de processus',

  'LBL_PMSE_PROJECT_FOCUS_DRAWER_DASHBOARD' => 'Tiroir de rangement Définition de processus',

  'LNK_LIST' => 'Afficher les définitions des processus',
  'LNK_NEW_PMSE_PROJECT' => 'Créer une définition de processus',
  'LNK_IMPORT_PMSE_PROJECT' => 'Importer une définition de processus',

  'LBL_PRJ_STATUS' => 'Statut',
  'LBL_PRJ_MODULE' => 'Module cible',
  'LBL_PRJ_RUN_ORDER' => 'Ordre d&#39;exécution',
  'LBL_PMSE_BUTTON_SAVE' => 'Sauvegarder',
  'LBL_PMSE_BUTTON_CANCEL' => 'Annuler',
  'LBL_PMSE_BUTTON_YES' => 'Oui',
  'LBL_PMSE_BUTTON_NO' => 'Non',
  'LBL_PMSE_BUTTON_OK' => 'OK',
    'LBL_PMSE_FORM_ERROR' => 'Veuillez résoudre les erreurs avant de poursuivre.',

    'LBL_PMSE_LABEL_DESIGN' => 'Modélisation',
    'LBL_PMSE_LABEL_EXPORT' => 'Export',
    'LBL_PMSE_LABEL_DELETE' => 'Supprimer',
    'LBL_PMSE_LABEL_ENABLE' => 'Activer',
    'LBL_PMSE_LABEL_DISABLE' => 'Désactiver',

    'LBL_PMSE_SAVE_DESIGN_BUTTON_LABEL' => 'Sauvegarder et modéliser',
    'LBL_PMSE_IMPORT_BUTTON_LABEL' => 'Import -',

    'LBL_PMSE_MY_PROCESS_DEFINITIONS' => 'Mes définitions de processus',
    'LBL_PMSE_ALL_PROCESS_DEFINITIONS' => 'Toutes les définitions des processus',

    'LBL_PMSE_PROCESS_DEFINITIONS_ENABLED' => 'Activé',
    'LBL_PMSE_PROCESS_DEFINITIONS_DISABLED' => 'Désactivé',
    'LBL_PMSE_PROCESS_DEFINITIONS_EDIT' => 'Il y a des processus actifs suivant cette définition du processus en cours. Une mise à jour de celle-ci risquerait de compromettre ces processus. Voulez-vous toujours modifier la définition de ce processus ?',
    'LBL_PMSE_DISABLE_CONFIRMATION_PD' => 'Il y a des processus actifs suivant cette définition du processus. Une mise à jour de celle-ci risquerait de compromettre ces processus. Voulez-vous toujours désactiver la définition de ce processus?',

    'LBL_PMSE_PROCESS_DEFINITION_IMPORT_TEXT' => 'Télécharger un fichier .bpm pour importer une définition de processus.',
    'LBL_PMSE_PROCESS_DEFINITION_IMPORT_SUCCESS' => 'La définition de processus a été importé correctement dans le système.',
    'LBL_PMSE_PROCESS_DEFINITION_EMPTY_WARNING' => 'Veuillez sélectionner un fichier *.bpm valide.',
    'LBL_PMSE_PROCESS_DEFINITION_IMPORT_BR' => 'La définition de processus a été importée, mais contient une ou plusieurs actions de règles de gestion pour lesquelles des règles de gestion n&#39;ont pas été sélectionnées.',
    'LBL_BPM_IMPORT_SELECT' => 'Cette définition de processus contient les règles de gestion et les modèles d&#39;emails suivants. '
        . 'Veuillez sélectionner ceux que vous voulez importer.',
    'LBL_PMSE_PROCESS_DEFINITION_IMPORT_ERROR' => 'There was an error importing the Process Definition.',
    'LBL_BUSINESS_RULES' => 'Règles de gestion',
    'LBL_EMAIL_TEMPLATES' => 'Modèles d&#39;email',


//    /*PMSE*/

//    'LBL_PMSE_LABEL_TERMINATE_CASES' => 'Terminate Process',

//    'LBL_PMSE_LABEL_CUSTOM_FORM_MODULE' => 'Custom Form Module',
//    'LBL_PMSE_LABEL_CUSTOM_FORM_PROCESS' => 'Custom Form Process',
//    'LBL_PMSE_LABEL_CUSTOM_FORM_NAME' => 'Custom Form name',
//    'LBL_PMSE_LABEL_CUSTOM_FORM_DESC' => 'Custom Form description',
//    'LBL_PMSE_LABEL_LOADING' => 'Loading, please wait...',
//    'LBL_PMSE_LABEL_CASETAKEN' => 'This Process was already taken by another User',
//    'LBL_PMSE_LABEL_CASECOMPLETED' => 'The Process is already closed',
//    'LBL_PMSE_LABEL_UNASSIGNED' => 'Unassigned',
//    'LBL_PMSE_LABEL_SEARCHBYDUEDATE' => 'Search By Due Date',
//    'LBL_PMSE_LABEL_SETTINGSEARCHBYDUEDATE' => 'Setting Search by  Due Date',
//    'LBL_PMSE_LABEL_SEARCH' => 'Search',
//    'LBL_PMSE_LABEL_DELETED_RECORD'=>'The records related to this Process has been removed',


    /**TOOLBAR**/
    'LBL_PMSE_ADAM_DESIGNER_LEADS' => 'Leads',
    'LBL_PMSE_ADAM_DESIGNER_OPPORTUNITY' => 'Affaire',
    'LBL_PMSE_ADAM_DESIGNER_DOCUMENTS' => 'Documents',
    'LBL_PMSE_ADAM_DESIGNER_OTHER_MODULES' => 'Module cible',
    'LBL_PMSE_ADAM_DESIGNER_WAIT' => 'Patienter',
    'LBL_PMSE_ADAM_DESIGNER_RECEIVE_MESSAGE' => 'Réception d&#39;un message',
    'LBL_PMSE_ADAM_DESIGNER_SEND_MESSAGE' => 'Envoi d&#39;un message',
    'LBL_PMSE_ADAM_DESIGNER_USER_TASK' => 'Activité',
    'LBL_PMSE_ADAM_DESIGNER_EXCLUSIVE' => 'Exclusif',
    'LBL_PMSE_ADAM_DESIGNER_PARALLEL' => 'Parallèle',
    'LBL_PMSE_ADAM_DESIGNER_COMMENT' => 'Commentaire',
    'LBL_PMSE_ADAM_DESIGNER_UNDO' => 'Annuler',
    'LBL_PMSE_ADAM_DESIGNER_REDO' => 'Réappliquer',
    'LBL_PMSE_ADAM_DESIGNER_SAVE' => 'Sauvegarder',
    'LBL_PMSE_ADAM_DESIGNER_VALIDATE' => 'Valider',
    'LBL_PMSE_ADAM_DESIGNER_SAVE_AND_VALIDATE' => 'Sauvegarder et valider',
    'LBL_PMSE_ADAM_DESIGNER_VIEW_ERRORS' => 'Exécuter la validation pour vérifier la présence d&#39;erreurs',

    /**ELEMENTS NAMES**/
    'LBL_PMSE_ADAM_DESIGNER_TASK' => 'Activité',
    'LBL_PMSE_ADAM_DESIGNER_ACTION' => 'Action',
    'LBL_PMSE_ADAM_DESIGNER_LEAD_START_EVENT' => 'Evènement sur un lead',
    'LBL_PMSE_ADAM_DESIGNER_OPPORTUNITY_START_EVENT' => 'Evènement sur une affaire' ,
    'LBL_PMSE_ADAM_DESIGNER_DOCUMENT_START_EVENT' => 'Evènement sur un document',
    'LBL_PMSE_ADAM_DESIGNER_OTHER_MODULE_EVENT' => 'Evènement sur un autre module',
    'LBL_PMSE_ADAM_DESIGNER_WAIT_EVENT' => 'Evènement sur une attente',
    'LBL_PMSE_ADAM_DESIGNER_MESSAGE_EVENT' => 'Evènement sur un message',
//    'LBL_PMSE_ADAM_DESIGNER_BOUNDARY_EVENT' => 'Boundary Event',
    'LBL_PMSE_ADAM_DESIGNER_EXCLUSIVE_GATEWAY' => 'Passerelle exclusive',
    'LBL_PMSE_ADAM_DESIGNER_PARALLEL_GATEWAY' => 'Passerelle parallèle',
    'LBL_PMSE_ADAM_DESIGNER_END_EVENT' => 'Événement de fin',
    'LBL_PMSE_ADAM_DESIGNER_TEXT_ANNOTATION' => 'Annotation',


    /**GENERAL**/
    'LBL_PMSE_MESSAGE_CANCEL_CONFIRM' => 'Certain paramètres ont changé. Souhaitez-vous annuler ces changements ?',
    'LBL_PMSE_MESSAGE_REMOVE_ALL_START_CRITERIA' => 'Le module a changé et par conséquence des critères seront supprimés étant donné qu&#39;il n&#39;y a plus de champs liés à ce module.',
    'LBL_PMSE_MESSAGE_INVALID_CONNECTION' => 'Connexion invalide',

    'LBL_PMSE_CONTEXT_MENU_SETTINGS' => 'Paramétrage...',
    'LBL_PMSE_CONTEXT_MENU_DELETE' => 'Supprimer',

    'LBL_PMSE_FORM_LABEL_MODULE' => 'Module',
    'LBL_PMSE_FORM_LABEL_FILTER' => 'Filtrer',
    'LBL_PMSE_FORM_LABEL_RELATED' => 'Associé à',
    'LBL_PMSE_FORM_LABEL_CRITERIA' => 'Critère',
    'LBL_PMSE_FORM_LABEL_DURATION' => 'Durée',
    'LBL_PMSE_FORM_LABEL_UNIT' => 'Unité de mesure',

    'LBL_PMSE_FORM_OPTION_SELECT' => 'Sélection...',
    'LBL_PMSE_FORM_OPTION_TARGET_MODULE' => 'Module cible',
    'LBL_PMSE_FORM_OPTION_DAYS' => 'Jours',
    'LBL_PMSE_FORM_OPTION_HOURS' => 'Heure(s)',
    'LBL_PMSE_FORM_OPTION_MINUTES' => 'Minute(s)',
    'LBL_PMSE_MESSAGE_CANNOTDROPOUTSIDECANVAS' => 'Impossible de supprimer l&#39;élément à l&#39;extérieur du canvas',
    'LBL_PMSE_FORM_TOOLTIP_DURATION' => 'Définir la durée du timer de l&#39;évènement',

    /**PROCESSDEFINTION**/
    // CONTEXT MENU
    'LBL_PMSE_CONTEXT_MENU_PROCESS_DEFINITION' => 'Définition de processus',
    'LBL_PMSE_CONTEXT_MENU_SAVE' => 'Sauvegarder',
    'LBL_PMSE_CONTEXT_MENU_REFRESH' => 'Rafraîchir',
    'LBL_PMSE_CONTEXT_MENU_ZOOM' => 'Zoom',
    'LBL_PMSE_CONTEXT_MENU_50' => '50%',
    'LBL_PMSE_CONTEXT_MENU_75' => '75%',
    'LBL_PMSE_CONTEXT_MENU_100' => '100 %',
    'LBL_PMSE_CONTEXT_MENU_125' => '125 %',
    'LBL_PMSE_CONTEXT_MENU_150' => '150 %',
    // FORMS
    'LBL_PMSE_LABEL_PROCESS_NAME' => 'Nom du processus',
    'LBL_PMSE_LABEL_DESCRIPTION' => 'Description',
    'LBL_PMSE_LABEL_TERMINATE_PROCESS' => 'Terminer le processus',
    'LBL_PMSE_LABEL_LOCKED_FIELDS' => 'Champs bloqués',

    /**TASKS**/
    // CONTEXT MENU
    'LBL_PMSE_CONTEXT_MENU_FORMS' => 'Formulaires...',
    'LBL_PMSE_CONTEXT_MENU_USERS' => 'Utilisateurs...',
    'LBL_PMSE_CONTEXT_MENU_ACTION_TYPE' => 'Type d&#39;Action',
    'LBL_PMSE_CONTEXT_MENU_UNASSIGNED' => '[Unassigned]',
    'LBL_PMSE_CONTEXT_MENU_BUSINESS_RULE' => 'Règle de gestion',
    'LBL_PMSE_CONTEXT_MENU_ASSIGN_USER' => 'Assigner un utilisateur',
    'LBL_PMSE_CONTEXT_MENU_ASSIGN_TEAM' => 'Round Robin',
    'LBL_PMSE_CONTEXT_MENU_CHANGE_FIELD' => 'Changer le champ',
    'LBL_PMSE_CONTEXT_MENU_ADD_RELATED_RECORD' => 'Ajouter un enregistrement liéé',
    // CONFIRMATIONS
    'LBL_PMSE_CHANGE_ACTION_TYPE_CONFIRMATION' => 'En changeant le type d&#39;action, tous les paramètres précédents pour cette action seront perdus. Vous voulez continuer ?',
    // FORMS
    'LBL_PMSE_FORM_TITLE_ACTIVITY' => 'Activité',
    'LBL_PMSE_FORM_LABEL_READ_ONLY_FIELDS' => 'Champs en lecture seule',
    'LBL_PMSE_FORM_LABEL_REQUIRED_FIELDS' => 'Champs obligatoires',
    'LBL_PMSE_FORM_LABEL_GENERAL_SETTINGS' => 'Général',
    'LBL_PMSE_FORM_LABEL_EXPECTED_TIME' => 'Durée prévue',
    'LBL_PMSE_FORM_LABEL_FORM_TYPE' => 'Type de formulaire',
    'LBL_PMSE_FORM_LABEL_RESPONSE_BUTTONS' => 'Buttons de formulaire',
    'LBL_PMSE_FORM_OPTION_APPROVE_REJECT' => 'Approuver / Rejetter',
    'LBL_PMSE_FORM_OPTION_ROUTE' => 'Route',
    'LBL_PMSE_FORM_LABEL_OTHER_DERIVATION_OPTIONS' => 'Autre options de routage',
    'LBL_PMSE_FORM_LABEL_RECORD_OWNERSHIP' => 'Changer le propriétaire de l&#39;enregistrement',
    'LBL_PMSE_FORM_LABEL_TEAM' => 'Équipe',
    'LBL_PMSE_FORM_LABEL_REASSIGN' => 'Réassigner l&#39;activité',
    'LBL_PMSE_FORM_LABEL_EMAIL_PROCESS_USER' => 'Envoyer un email',
    'LBL_PA_FORM_LABEL_EMAIL_PROCESS_USER' => 'Envoyer un e-mail à l&#39;utilisateur du processus lorsqu&#39;un processus est attribué',

    'LBL_PMSE_FORM_TITLE_USER_DEFINITION' => 'Définition utilisateur',
    'LBL_PMSE_FORM_LABEL_ASSIGNMENT_METHOD' => 'Méthode d&#39;assignation',
    'LBL_PMSE_FORM_OPTION_ROUND_ROBIN' => 'Round Robin',
    'LBL_PMSE_FORM_OPTION_SELF_SERVICE' => 'Libre service',
    'LBL_PMSE_FORM_OPTION_STATIC_ASSIGNMENT' => 'Assignation statique',
    'LBL_PMSE_FORM_LABEL_ASSIGN_TO_TEAM' => 'Assigné à l&#39;équipe',
    'LBL_PMSE_FORM_LABEL_ASSIGN_TO_USER' => 'Assigné à l&#39;utilisateur',
    'LBL_PA_FORM_COMBO_ASSIGN_TO_USER_HELP_TEXT' => 'Sélection...',
    'LBL_PA_FORM_COMBO_NO_MATCHES_FOUND' => 'Aucune correspondance trouvée',
    'LBL_PA_FORM_LABEL_ASSIGN_TO_TEAM' => 'Sélectionnez l&#39;utilisateur du processus dans l&#39;équipe',
    'LBL_PA_FORM_LABEL_ASSIGN_TO_USER' => 'Sélectionnez l&#39;utilisateur du processus',
    'LBL_PMSE_FORM_OPTION_CURRENT_USER' => 'Utilisateur actuel',
    'LBL_PMSE_FORM_OPTION_RECORD_OWNER' => 'Propriétaire de l&#39;enregistrement',
    'LBL_PMSE_FORM_OPTION_SUPERVISOR' => 'Superviseur',
    'LBL_PMSE_FORM_OPTION_CREATED_BY_USER' => 'Créé par l&#39;utilisateur',
    'LBL_PMSE_FORM_OPTION_LAST_MODIFIED_USER' => 'Dernière modification par l&#39;utilisateur',
    'LBL_PMSE_FORM_OPTION_SYSTEM_EMAIL' => 'E-mail de système',

    'LBL_PMSE_FORM_TITLE_BUSINESS_RULE' => 'Règle de gestion',
    'LBL_PMSE_LABEL_RULE' => 'Règle',

    'LBL_PMSE_FORM_TITLE_ASSIGN_USER' => 'Assigner un utilisateur',
    'LBL_PA_FORM_LABEL_UPDATE_RECORD_OWNER' => 'Mise à jour de l&#39;enregistrement sur « Assigné à »',
    'LBL_PA_FORM_LABEL_SET_BY_AVAILABILITY' => 'Définir "Assigné à" en fonction de la disponibilité',

    'LBL_PMSE_FORM_TITLE_ADD_RELATED_RECORD' => 'Ajouter un enregistrement liéé',
    'LBL_PMSE_FORM_LABEL_RELATED_MODULE' => 'Module Associé',
    'LBL_PMSE_FORM_LABEL_FIELDS' => 'Champs',

    'LBL_PMSE_FORM_TITLE_CHANGE_FIELDS' => 'Changer les champs',

    'LBL_PMSE_FORM_TITLE_ASSIGN_TEAM' => 'Round Robin',

    'LBL_PMSE_MESSAGE_ACTIVITY_NAME_EMPTY' => 'Le nom de l&#39;activité est vide.',
    'LBL_PMSE_MESSAGE_ACTIVITY_NAME_ALREADY_EXISTS' => 'Le nom "%s" existe déjà dans la famille des formes disponibles.',

    'LBL_PMSE_FORM_REQUIRED_SHIFT_AVAILABILITY' => 'Disponibilité de la période de travail requise',
    'LBL_PMSE_FORM_LABEL_IF_NO_AVAILABLE' => 'Si aucun utilisateur n&#39;est disponible',

    'LBL_PMSE_FORM_LABEL_BEFORE' => 'avant',

    /**EVENTS**/
    // CONTEXT MENU
    'LBL_PMSE_CONTEXT_MENU_ACTION' => 'Action',
    'LBL_PMSE_CONTEXT_MENU_RECEIVE_MESSAGE' => 'Réception d&#39;un message',
    'LBL_PMSE_CONTEXT_MENU_SEND_MESSAGE' => 'Envoi d&#39;un message',
    'LBL_PMSE_CONTEXT_MENU_TIMER' => 'Minuterie',
    'LBL_PMSE_CONTEXT_MENU_RESULT' => 'Résultat',
    'LBL_PMSE_CONTEXT_MENU_DO_NOTHING' => 'Ne rien faire',
    'LBL_PMSE_CONTEXT_MENU_TERMINATE_PROCESS' => 'Terminer le processus',
    'LBL_PMSE_CONTEXT_MENU_LISTEN' => 'Ecoute',

    // FORMS
    'LBL_PMSE_FORM_TITLE_LABEL_EVENT' => 'Evènement',
    'LBL_PMSE_FORM_LABEL_APPLIES_TO' => 'Appliquer à',
    'LBL_PMSE_FORM_OPTION_NEW_RECORDS_ONLY' => 'Nouveaux enregistrements seulement',
    'LBL_PMSE_FORM_OPTION_UPDATED_RECORDS_ONLY' => 'A la mise à jour des enregistrements seulement',
    'LBL_PMSE_FORM_OPTION_UPDATED_RECORDS_ONLY_AU' => 'Enregistrements mis à jours (Tout mettre à jour - Afficher l&#39;aide)',
    'LBL_PMSE_FORM_OPTION_NEW_AND_FIRST_UPDATED_RECORDS' => 'Nouveaux enregistrements ou première mise à jour',
    'LBL_PMSE_FORM_OPTION_NEW_AND_ALL_UPDATED_RECORDS' => 'Nouveaux enregistrements et toutes les mises à jour',

    'LBL_PMSE_FORM_TOOLTIP_WHEN_START_EVENT' => 'Sélectionner le moment où le processus démarrera.<br><br>Nouveau : Exécuter le processus lors de la création d&#39;un nouvel enregistrement.<br><br>Première mise à jour : le processus ne s&#39;exécute que lorsque le critère de première mise à jour est rempli pour un enregistrement existant.<br><br>Toutes les mises à jour : Le processus s&#39;exécute chaque fois que le critère est rempli pour un enregistrement existant.<br><br>Nouveaux enregistrements ou première mise à jour : Le processus s&#39;exécute uniquement lorsque le critère est rempli pour la première fois pour un enregistrement, que ce soit lors de la création ou de la mise à jour d&#39;un enregistrement.<br><br>Nouveaux enregistrements et toutes les mises à jour : Le processus s&#39;exécute à chaque fois pour tous les nouveaux enregistrements et les enregistrements existants.',
    'LBL_PMSE_FORM_TOOLTIP_EVENT_MODULE' => 'Sélectionner le module Sugar déclenchant l&#39;évènement',

    'LBL_PMSE_FORM_LABEL_FIXED_DATE' => 'Date fixe',

    'LBL_PMSE_FORM_LABEL_EMAIL_TEMPLATE' => 'Modèle Email',

    'LBL_PMSE_FORM_LABEL_EMAIL_TO' => 'A',
    'LBL_PMSE_FORM_LABEL_EMAIL_CC' => 'Cc',
    'LBL_PMSE_FORM_LABEL_EMAIL_BCC' => 'Cci',

    //ROLES IN EXPRESSION BUILDER
    'LBL_PMSE_FORM_OPTION_ADMINISTRATOR' => 'Administrateur',

    /**GATEWAYS**/
    // CONTEXT MENU
    'LBL_PMSE_CONTEXT_MENU_DIRECTION' => 'Direction...',
    'LBL_PMSE_CONTEXT_MENU_CONVERGING' => 'Convergent',
    'LBL_PMSE_CONTEXT_MENU_DIVERGING' => 'Divergent',
    'LBL_PMSE_CONTEXT_MENU_CONVERT' => 'Convertir...',
    'LBL_PMSE_CONTEXT_MENU_EXCLUSIVE_GATEWAY' => 'Passerelle exclusive',
    'LBL_PMSE_CONTEXT_MENU_PARELLEL_GATEWAY' => 'Passerelle parallèle',
    'LBL_PMSE_CONTEXT_MENU_INCLUSIVE_GATEWAY' => 'Passerelle inclue',
    'LBL_PMSE_CONTEXT_MENU_EVENT_BASED_GATEWAY' => 'Passerelle basé sur les évènements',
    'LBL_PMSE_CONTEXT_MENU_DEFAULT_FLOW' => 'Flux par défaut',
    'LBL_PMSE_CONTEXT_MENU_NONE' => 'Aucun',

    // FORMS
    'LBL_PMSE_FORM_TITLE_GATEWAY' => 'Passerelle',

    'LBL_PMSE_CONTEXT_MENU_DEFAULT_TASK' => '(Activité)',
    'LBL_PMSE_CONTEXT_MENU_DEFAULT_EVENT' => '(Evènement)',
    'LBL_PMSE_CONTEXT_MENU_DEFAULT_GATEWAY' => '(Passerelle)',

    'LBL_PMSE_BPMN_WARNING_PANEL_TITLE' => 'Eléments en erreurs',
    'LBL_PMSE_BPMN_WARNING_LABEL' => 'Erreurs',
    'LBL_PMSE_BPMN_WARNING_SINGULAR_LABEL' => 'Erreur',

    /**CRITERIA BUILDER**/
    'LBL_PMSE_EXPCONTROL_VARIABLES_PANEL_TITLE' => 'Champs',
    'LBL_PMSE_EXPCONTROL_MODULE_FIELD_EVALUATION_TITLE' => 'Évaluation de champ du module',
    'LBL_PMSE_EXPCONTROL_MODULE_FIELD_EVALUATION_MODULE' => 'Module',
    'LBL_PMSE_EXPCONTROL_MODULE_FIELD_EVALUATION_RELATED' => 'Lié à',
    'LBL_PMSE_EXPCONTROL_MODULE_FIELD_EVALUATION_VARIABLE' => 'Champ',
    'LBL_PMSE_EXPCONTROL_MODULE_FIELD_EVALUATION_VALUE' => 'Valeur',
    'LBL_PMSE_EXPCONTROL_FORM_RESPONSE_EVALUATION_TITLE' => 'Évaluation de réponse de formulaire',
    'LBL_PMSE_EXPCONTROL_FORM_RESPONSE_EVALUATION_FORM' => 'Formulaire',
    'LBL_PMSE_EXPCONTROL_FORM_RESPONSE_EVALUATION_STATUS' => 'Statut',
    'LBL_PMSE_EXPCONTROL_BUSINESS_RULES_EVALUATION_TITLE' => 'Évaluation des règles de gestion',
    'LBL_PMSE_EXPCONTROL_BUSINESS_RULES_EVALUATION_BR' => 'Règle de gestion',
    'LBL_PMSE_EXPCONTROL_BUSINESS_RULES_EVALUATION_RESPONSE' => 'Réponse',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_TITLE' => 'Évaluation utilisateur',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_USER' => 'Assigné à',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_CURRENT' => 'Utilisateur courant',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_SUPERVISOR' => 'Superviseur',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_OWNER' => 'Propriétaire de l&#39;enregistrement',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_OPERATOR' => 'Opérateur',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_ADMIN' => 'est admin',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_ADMIN_FULL' => '%TARGET% est admin',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_ROLE' => 'a le rôle de',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_ROLE_FULL' => '%TARGET% a le rôle de %VALUE%',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_USER' => 'est utilisateur',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_USER_FULL' => '%TARGET% est l&#39;utilisateur %VALUE%',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_NOT_ADMIN' => 'n&#39;est pas admin',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_NOT_ADMIN_FULL' => '%TARGET% n&#39;est pas admin',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_NOT_ROLE' => 'n&#39;a pas le rôle de',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_NOT_ROLE_FULL' => '%TARGET% n&#39;a pas le rôle de %VALUE%',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_NOT_USER' => 'n&#39;est pas utilisateur',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_IS_NOT_USER_FULL' => '%TARGET% n&#39;est pas l&#39;utilisateur %VALUE%',
    'LBL_PMSE_EXPCONTROL_USER_EVALUATION_VALUE' => 'Valeur',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_FIXED_DATE' => 'Date fixe',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_FIXED_DATETIME' => 'Date fixe',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_TITLE' => 'Intervalle de temps',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_AMOUNT' => 'Valeur',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_UNIT' => 'Unité de mesure',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_YEARS' => 'années',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_MONTHS' => 'mois',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_WEEKS' => 'semaines',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_DAYS' => 'jours',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_HOURS' => 'heures',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_MINUTES' => 'minutes',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_BUSINESS_HOURS' => 'heures d&#39;ouverture',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_BUSINESS_CENTER' => 'Centre d&#39;affaires',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_BUSINESS_CENTER_FROM_TARGET_MODULE' => 'Du module cible',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_BUSINESS_CENTER_FROM' => 'De ',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TIMESPAN_BUSINESS_CENTER_MODULE' => ' Module',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_BASIC' => 'Chaîne, nombre et boolean',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_BASIC_NUMBER' => 'Numéro',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_BASIC_VALUE' => 'Valeur',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_BASIC_ADD_STRING' => 'et chaîne',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_BASIC_ADD_NUMBER' => 'et nombre',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_BASIC_ADD_BOOLEAN' => 'et boolean',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_CURRENCY' => 'Devise',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_CURRENCY_CURRENCY' => 'Devise',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_CURRENCY_AMOUNT' => 'Montant',
    'LBL_PMSE_EXPCONTROL_EVALUATIONS_TITLE' => 'Evaluation',
    'LBL_PMSE_EXPCONTROL_CONSTANTS_TITLE' => 'Contantes',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MINOR_THAN' => 'Est inférieur à',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MINOR_THAN_DATE' => 'Avant',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MINOR_EQUAL_THAN' => 'Est inférieur ou égal à',
    'LBL_PMSE_EXPCONTROL_OPERATOR_EQUAL' => 'Egal',
    'LBL_PMSE_EXPCONTROL_OPERATOR_EQUAL_TEXT' => 'Est',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MAJOR_EQUAL' => 'Est supérieur ou égal à',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MAJOR' => 'Est supérieur à',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MAJOR_DATE' => 'Après',
    'LBL_PMSE_EXPCONTROL_OPERATOR_STARTS_TEXT' => 'commence par',
    'LBL_PMSE_EXPCONTROL_OPERATOR_ENDS_TEXT' => 'se termine par',
    'LBL_PMSE_EXPCONTROL_OPERATOR_CONTAINS_TEXT' => 'contient',
    'LBL_PMSE_EXPCONTROL_OPERATOR_NOT_CONTAINS_TEXT' => 'ne contient pas',

    'LBL_PMSE_EXPCONTROL_OPERATOR_CHANGES' => 'modifications',
    'LBL_PMSE_EXPCONTROL_OPERATOR_CHANGES_FROM' => 'modifications pour',
    'LBL_PMSE_EXPCONTROL_OPERATOR_CHANGES_TO' => 'modifications vers',

    'LBL_PMSE_EXPCONTROL_OPERATOR_MULTISELECT_IS_ON_OF' => 'contient',

    'LBL_PMSE_EXPCONTROL_OPERATOR_MAJOR_EQUAL_DATE' => 'le ou après',
    'LBL_PMSE_EXPCONTROL_OPERATOR_MINOR_EQUAL_DATE' => 'le ou avant',
    'LBL_PMSE_EXPCONTROL_OPERATOR_NOT_EQUAL' => 'Différent de',
    'LBL_PMSE_EXPCONTROL_OPERATOR_NOT_EQUAL_TEXT' => 'N&#39;est pas',
    'LBL_PMSE_EXPCONTROL_OPERATOR_NOT_EQUAL_DATE' => 'pas égal à',

    'LBL_PMSE_EXPCONTROL_ALL_RELATED_RECORDS' => 'Tous les enregistrements liés',
    'LBL_PMSE_EXPCONTROL_ANY_RELATED_RECORDS' => 'Tout enregistrement lié',

    'LBL_PMSE_RUNTIME_BUTTON' => 'Temps d&#39;exécution',

    'LBL_PMSE_FORMPANEL_SUBMIT' => 'Ajouter',
    'LBL_PMSE_FORMPANEL_CLOSE' => 'Fermer',

    'LBL_PMSE_EMAILPICKER_ALL_ASSIGNED_TEAMS' => 'Toutes les équipes assignées à l&#39;enregistrement',
    'LBL_PMSE_EMAILPICKER_TEAMS' => 'Équipes',
    'LBL_PMSE_EMAILPICKER_RELATED_TO' => '%RELATED% lié à %MODULE%',
    'LBL_PMSE_EMAILPICKER_USER_CREATED' => 'Utilisateur qui a créé le %MODULE%',
    'LBL_PMSE_EMAILPICKER_USER_LAST_MODIFIED' => 'Utilisateur qui a modifié le %MODULE% en dernier',
    'LBL_PMSE_EMAILPICKER_USER_IS_ASSIGNED' => 'Utilisateur qui est affecté au %MODULE%',
    'LBL_PMSE_EMAILPICKER_USER_WAS_ASSIGNED' => 'Utilisateur qui était affecté au %MODULE%',
    'LBL_PMSE_EMAILPICKER_MANAGER_CREATED' => 'Responsable qui a créé le %MODULE%',
    'LBL_PMSE_EMAILPICKER_MANAGER_LAST_MODIFIED' => 'Responsable qui a modifié le %MODULE% en dernier',
    'LBL_PMSE_EMAILPICKER_MANAGER_IS_ASSIGNED' => 'Responsable qui est affecté au %MODULE%',
    'LBL_PMSE_EMAILPICKER_MANAGER_WAS_ASSIGNED' => 'Responsable qui était affecté au %MODULE%',
    'LBL_PMSE_EMAILPICKER_ROLE_ITEM' => 'rôle : %ROLE%',
    'LBL_PMSE_EMAILPICKER_TEAM_ITEM' => 'équipe : %TEAM%',
    'LBL_PMSE_EMAILPICKER_SUGGESTIONS' => 'Suggestions',
    'LBL_PMSE_EMAILPICKER_RESULTS_TITLE' => '%NUMBER% suggestion(s) pour "%TEXT%"',
    'LBL_PMSE_EMAILPICKER_USER_RECORD_CREATOR' => 'a créé le dossier',
    'LBL_PMSE_EMAILPICKER_USER_LAST_MODIFIER' => 'a modifié l&#39;enregistrement en dernier',
    'LBL_PMSE_EMAILPICKER_USER_IS_ASIGNEE' => 'est affecté à l&#39;enregistrement',

    'LBL_PMSE_UPDATERFIELD_VARIABLES_LIST_TITLE' => 'champs du module %MODULE%',
    'LBL_PMSE_UPDATERFIELD_ADD_TEAM' => 'Ajouter une équipe...',

    //ERRORS ELEMENTS MESSAGE
    'LBL_PMSE_MESSAGE_ERROR_START_EVENT_OUTGOING' => 'Un événement de début doit avoir un flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_END_EVENT_INCOMING' => 'Un événement de fin doit avoir un flux d&#39;entrée',
    'LBL_PMSE_MESSAGE_ERROR_INTERMEDIATE_EVENT_INCOMING' => 'Un événement de fin doit avoir un flux ou plusieurs flux de d&#39;entrée',
    'LBL_PMSE_MESSAGE_ERROR_INTERMEDIATE_EVENT_OUTGOING' => 'Un événement de fin doit avoir un ou plusieurs flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_BOUNDARY_EVENT_OUTGOING' => 'Un événement d&#39;attente de début doit avoir un flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_ACTIVITY_INCOMING' => 'Une activité doit avoir un flux d&#39;entrée',
    'LBL_PMSE_MESSAGE_ERROR_ACTIVITY_OUTGOING' => 'Une activité doit avoir un flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_ACTIVITY_SCRIPT_TASK' => 'La tâche de script doit être d&#39;un type valide différent de [Unassigned]',
    'LBL_PMSE_MESSAGE_ERROR_GATEWAY_DIVERGING_INCOMING' => 'Une passerelle peut avoir un ou plusieurs flux d&#39;entrée',
    'LBL_PMSE_MESSAGE_ERROR_GATEWAY_DIVERGING_OUTGOING' => 'Une passerelle doit avoir au moins deux flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_GATEWAY_CONVERGING_INCOMING' => 'Une passerelle doit avoir au moins deux flux d&#39;entrée',
    'LBL_PMSE_MESSAGE_ERROR_GATEWAY_CONVERGING_OUTGOING' => 'Une passerelle ne peut pas avoir un seul flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_GATEWAY_MIXED_INCOMING' => 'Une passerelle doit avoir au moins deux flux d&#39;entrée',
    'LBL_PMSE_MESSAGE_ERROR_GATEWAY_MIXED_OUTGOING' => 'Une passerelle doit avoir au moins deux flux de sortie',
    'LBL_PMSE_MESSAGE_ERROR_ANNOTATION' => 'Une annotation doit avoir une ligne d&#39;association',

    'LBL_PMSE_IMPORT_PROCESS_DEFINITION_FAILURE' => 'La création de définition de processus à partir du fichier a échoué',
    'LBL_PMSE_CANNOT_CONFIGURE_ADD_RELATED_RECORD' => 'Il n&#39;existe aucun module associé pour ce module cible',
    'LBL_PMSE_PROJECT_NAME_EMPTY' => 'Le nom de définition du processus ne doit pas être enregistré non rempli car ce champ est obligatoire.',

    'LBL_PMSE_INVALID_EXPRESSION_SYNTAX' => 'Syntaxe de l&#39;expression non valide.',
    'LBL_PMSE_MESSAGE_ERROR_CURRENCIES_MIX' => 'Can&#39;t use two different currencies in the same expression.',

    // PMSE Validation tool strings

    'LBL_PMSE_VALIDATOR_IN_PROGRESS_RETRIEVING' => 'Validation de la définition du processus : Récupération des réglages des éléments',
    'LBL_PMSE_VALIDATOR_IN_PROGRESS_VALIDATING' => 'Validation de la définition du processus : Validation des paramètres des éléments',
    'LBL_PMSE_VALIDATOR_REFRESH_ERROR_LIST' => 'Actualisation de la liste des erreurs...',
    'LBL_PMSE_VALIDATOR_COMPLETE' => 'Validation terminée ! Problèmes trouvés : ',
    'LBL_PMSE_VALIDATOR_TOOLTIP_ISSUES' => ' problèmes',
    'LBL_PMSE_VALIDATOR_TOOLTIP_IN_PROGRESS' => 'Validation en cours',
    'LBL_PMSE_VALIDATOR_WARNING_INFO' => 'Attention : Le processus peut s&#39;arrêter à cet élément ou avoir d&#39;autres effets involontaires',
    'LBL_PMSE_VALIDATOR_ERROR_INFO' => 'Erreur : Le processus s&#39;arrêtera à cet élément',

    'LBL_PMSE_ERROR_UNABLE_TO_VALIDATE' => 'Impossible de valider l&#39;élément',

    'LBL_PMSE_ERROR_FLOW_INCOMING_MINIMUM' => 'L&#39;élément ne répond pas au nombre minimum de flux d&#39;entrée',
    'LBL_PMSE_ERROR_FLOW_INCOMING_MINIMUM_INFO' => 'Cet élément doit avoir un nombre minimum de chemins d&#39;accès entrants. Vérifiez le nombre de chemins qui se connectent à cet élément pour s&#39;assurer que le nombre minimum est atteint.',

    'LBL_PMSE_ERROR_FLOW_INCOMING_MAXIMUM' => 'L&#39;élément dépasse le nombre maximum de flux d&#39;entrée',
    'LBL_PMSE_ERROR_FLOW_INCOMING_MAXIMUM_INFO' => 'Cet élément a un nombre maximum de chemins d&#39;accès entrants qui ne doivent pas être dépassés. Vérifiez le nombre de chemins qui se connectent à cet élément pour s&#39;assurer que le nombre maximum n&#39;est pas dépassé.',

    'LBL_PMSE_ERROR_FLOW_OUTGOING_MINIMUM' => 'L&#39;élément ne répond pas au nombre minimum de flux de sortie',
    'LBL_PMSE_ERROR_FLOW_OUTGOING_MINIMUM_INFO' => 'Cet élément doit avoir un nombre minimum de chemins de sortie. Vérifiez le nombre de chemins sortant de cet élément pour s&#39;assurer que le nombre minimum est atteint.',

    'LBL_PMSE_ERROR_FLOW_OUTGOING_MAXIMUM' => 'L&#39;élément dépasse le nombre maximum de flux de sortie',
    'LBL_PMSE_ERROR_FLOW_OUTGOING_MAXIMUM_INFO' => 'Cet élément a un nombre maximum de chemins de sortie qui ne doit pas être dépassé. Vérifiez le nombre de chemins sortant de cet élément pour s&#39;assurer que le nombre maximum n&#39;est pas dépassé.',

    'LBL_PMSE_ERROR_FIELD_REQUIRED' => 'Le champ obligatoire n&#39;est pas défini',
    'LBL_PMSE_ERROR_FIELD_REQUIRED_INFO' => 'Cet élément possède un champ qui est obligatoire, mais qui n&#39;a pas été défini. Vérifiez les réglages des éléments pour s&#39;assurer que le champ a été correctement défini, puis s&#39;assurer de cliquer sur le bouton « Enregistrer ».',

    'LBL_PMSE_ERROR_DATA_NOT_FOUND' => 'Les données n&#39;existent pas actuellement dans le système',
    'LBL_PMSE_ERROR_DATA_NOT_FOUND_INFO' => 'Les paramètres de cet élément font référence à une donnée spécifique qui n&#39;existe pas dans la base de données. Vérifiez les paramètres des éléments pour s&#39;assurer que toutes les données référencées existent (c.-à-d. utilisateurs spécifiques, champs de module, équipes, etc.).',

    'LBL_PMSE_ERROR_LOGIC_IMPOSSIBLE' => 'L&#39;expression de la case de critères ne sera jamais évaluée comme vrai',
    'LBL_PMSE_ERROR_LOGIC_IMPOSSIBLE_INFO' => 'Les cases de critères contiennent des expressions logiques ET/OU/OU/NON. Cette erreur signifie qu&#39;une case de critères dans les paramètres de l&#39;élément a été configurée de telle sorte que l&#39;expression ne sera jamais vraie. Cochez la case des critères pour éliminer toute contradiction logique qui empêche l&#39;expression d&#39;être vraie.',

    'LBL_PMSE_ERROR_ELEMENT_UNREACHABLE' => 'L&#39;élément n&#39;est pas accessible',
    'LBL_PMSE_ERROR_ELEMENT_UNREACHABLE_INFO' => 'Pour qu&#39;un élément puisse être exécuté dans un processus, il doit avoir un chemin d&#39;accès qui lui est connecté. La chaîne des chemins menant à cet élément doit commencer par un élément de départ. Vérifiez la chaîne des chemins d&#39;accès entrants à cet élément pour vous assurer que la chaîne commence par un élément de départ.',

    'LBL_PMSE_ERROR_WAIT_EVENT_ONE_DATETIME' => 'Les critères de temps d&#39;attente doivent contenir exactement une constante de date',
    'LBL_PMSE_ERROR_WAIT_EVENT_ONE_DATETIME_INFO' => 'Cet élément d&#39;événement d&#39;attente a été configuré à l&#39;aide de la case de critères « Date fixe ». Cette case doit contenir exactement une constante « Date fixe » ou une référence de champ Date fixe. Tous les autres éléments de l&#39;expression doivent être des opérateurs mathématiques (+/-) ou des constantes « Intervalle de temps».',

    'LBL_PMSE_ERROR_WAIT_EVENT_ZERO_DURATION' => 'La valeur de durée spécifiée ne doit pas être égale à zéro',
    'LBL_PMSE_ERROR_WAIT_EVENT_ZERO_DURATION_INFO' => 'Cet élément d&#39;événement d&#39;attente a été configuré à l&#39;aide du paramètre « Durée ». Ce réglage ne doit pas être égal à zéro, sinon une erreur se produira. Vérifiez les réglages des éléments pour vous assurer que la « Durée » n&#39;est pas définie sur zéro.',

    'LBL_PMSE_ERROR_WAIT_EVENT_NO_PARAMETERS' => 'Les paramètres de temps d&#39;attente n&#39;ont pas été définis',
    'LBL_PMSE_ERROR_WAIT_EVENT_NO_PARAMETERS_INFO' => 'Cet élément d&#39;événement d&#39;attente n&#39;a pas de paramètres configurés. Vérifiez les paramètres de l&#39;élément pour vous assurer que « Durée » ou « Date fixe » est sélectionné, entrez les valeurs souhaitées, puis cliquez sur le bouton « Enregistrer ».',

    'LBL_PMSE_ERROR_ACTIVITY_EXPECTED_TIME' => 'L&#39;heure prévue est inférieure à zéro',
    'LBL_PMSE_ERROR_ACTIVITY_EXPECTED_TIME_INFO' => 'Dans les paramètres « Formulaires » de cet élément d&#39;activité, l&#39;heure prévue est réglée sur une valeur négative, ce qui provoque le gel du processus lorsque l&#39;élément est atteint. Ouvrez les paramètres « Formulaires » de cet élément pour changer l&#39;heure prévue en un nombre non négatif.',

    'LBL_PMSE_ERROR_ACTION_UNASSIGNED' => 'Le type d&#39;action est [Unassigned]',
    'LBL_PMSE_ERROR_ACTION_UNASSIGNED_INFO' => 'Cet élément d&#39;action n&#39;a pas de type désigné. Ceci peut être modifié en cliquant avec le bouton droit de la souris sur l&#39;élément d&#39;action, puis en sélectionnant un type dans la liste « Type d&#39;action ».',

    'LBL_PMSE_ERROR_GATEWAY_NO_GUARANTEED_PATH' => 'Il n&#39;est pas garanti qu&#39;un chemin sera emprunté',
    'LBL_PMSE_ERROR_GATEWAY_NO_GUARANTEED_PATH_INFO' => 'Ce message d&#39;erreur se produit parce qu&#39;il y a une chance qu&#39;aucun des chemins de sortie de cet élément de passerelle ne soit pris, ce qui entraînera l&#39;arrêt du processus. Il est conseillé de définir un chemin par défaut au cas où aucune des expressions des cases de critères dans les paramètres de l&#39;élément n&#39;est satisfaite. Pour ce faire, cliquez avec le bouton droit de la souris sur l&#39;élément de la passerelle et sélectionnez un élément dans le menu "Flux par défaut". Si aucun chemin par défaut n&#39;est spécifié, les expressions de la case des critères dans les paramètres de l&#39;élément doivent être modifiées pour s&#39;assurer qu&#39;au moins l&#39;un d&#39;eux sera vrai lorsque le processus sera exécuté.',

    'LBL_PMSE_ERROR_GATEWAY_CONVERGING_TYPE_MISMATCH' => 'Le type de passerelle convergente ne correspond pas au type de passerelle d&#39;un flux d&#39;entrée',
    'LBL_PMSE_ERROR_GATEWAY_CONVERGING_TYPE_MISMATCH_INFO' => 'Des passerelles divergentes étendent les processus à plusieurs branches. Lors de l&#39;utilisation d&#39;une passerelle convergente pour faire converger de nouveau ces branches dans un chemin, le type de passerelle convergente approprié doit être utilisé, en fonction du type de passerelle divergente qui a servi à créer les branches. Pour les branches créées avec des passerelles divergentes exclusives ou basées sur des événements, utilisez une passerelle convergente exclusive pour les faire converger de nouveau. Pour les branches créées avec des passerelles parallèles ou inclusives divergentes, utilisez une passerelle parallèle convergente pour les faire converger de nouveau.',
);
