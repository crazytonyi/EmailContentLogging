...
{
    global $sugar_config;
    if (empty($sugar_config['enable_CSP'])) {
        return;
    }
    $administration = Administration::getSettings('csp');
    $settings = $administration->settings;
    $cspDefaults = getDefaultCSPFields();
    ...
    $cspSettings = array_merge($cspDefaults, $cspSettings);
    $cspSettings['csp_child_src'] = $cspSettings['csp_frame_src'];
    foreach ($cspSettings as $k => $v) {
        $directive = str_replace(['csp_', '_'], ['', '-'], $k);
        if (empty($administration->settings[$k]) && $directive !== 'default-src') {
            continue;
        }
        if (isset($cspDefaults[$k])) {
            $cspDirectives[] = $directive . ' ' . sanitizeCspDirectiveValue($cspDefaults[$k] . ' ' . $v);
        }
    }
    header('Content-Security-Policy: ' . implode('; ', $cspDirectives));
}

function sanitizeCspDirectiveValue(string $cspDirective): string
{
    $normalizedDirective = str_replace(['  ', ';'], [' ', ''], htmlspecialchars_decode($cspDirective, ENT_QUOTES));
    $values = explode(' ', $normalizedDirective);
    $uniqueValues = array_unique($values);
    return implode(' ', $uniqueValues);
}

function isValidCSPSrcValue(string $cspDirective): bool
{
    $regex = <<<'regex'
~
    (?(DEFINE)
...
~ix
regex;
    return (bool) preg_match($regex, $cspDirective);
}

function getDefaultCSPFields(): array
{
    $cspDefaults['csp_default_src'] = "'self' data: 'unsafe-inline' 'unsafe-eval' https://*.pendo.io *.sugarcrm.com *.salesfusion.com *.salesfusion360.com *.sugarapps.com *.sugarapps.eu";
    $cspDefaults['csp_style_src'] = "'self' 'unsafe-inline'";
    $cspDefaults['csp_script_src'] = "'self' 'unsafe-inline' 'unsafe-eval' https://*.pendo.io";
    $cspDefaults['csp_img_src'] = "http: https: data:";
    $cspDefaults['csp_base_uri'] = "'self'";
    $cspDefaults['csp_form_action'] = "'self'";
    $cspDefaults['csp_object_src'] = "'self'";
    $cspDefaults['csp_frame_src'] = "'self'";
    $cspDefaults['csp_child_src'] = "'self'";
    $cspDefaults['csp_connect_src'] = "'self' https://*.pendo.io";
    $cspDefaults['csp_font_src'] = "'self'";
    $cspDefaults['csp_frame_ancestors'] = "'self'";
    return $cspDefaults;
}
