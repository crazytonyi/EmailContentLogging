...
return{isValid:true,error:''};},createUserSrn:function(userId){let srnParts=app.config.tenant.split(':').slice(0,5);srnParts.splice(3,1,'');srnParts.push('user',userId);return srnParts.join(':');},getUnderscoreToCamelCaseString:function(str){return str.replace(/_(\D)/g,function(a,b){return b.toUpperCase();});},isFieldAlwaysReadOnly:function(fieldDef,viewDef){if(_.isUndefined(viewDef)||_.isUndefined(viewDef.readonly)){return app.utils.isTruthy(fieldDef.readonly)&&_.isUndefined(fieldDef.readonly_formula);}else{return app.utils.isTruthy(viewDef.readonly);}},updatePendoMetadata:function(addCustomFieldToVisitor,addCustomFieldToAccount){var pendoObject=app.analytics.connectors.Pendo.getPendoMetadata();var updatedVisitorObject=addCustomFieldToVisitor?_.extend(pendoObject.visitor,addCustomFieldToVisitor):pendoObject.visitor;var updatedAccountObject=addCustomFieldToAccount?_.extend(pendoObject.account,addCustomFieldToAccount):pendoObject.account;pendo.updateOptions({visitor:updatedVisitorObject,account:updatedAccountObject});},deleteFileFromField:function(field,data,shouldRender){callbacks={success:function(){field.model.set(data.field,'');field.model.save({},{showAlerts:{'process':true,'success':{messages:app.lang.get('LBL_FILE_DELETED')}},fields:[data.field]});if(self.disposed){return;}
if(shouldRender){field.render();}},error:function(data){app.error.handleHttpError(data,{});}};app.api.file('delete',data,null,callbacks,{htmlJsonFormat:false});}});});})(SUGAR.App);
...
url='https://www.sugarcrm.com/crm/product_doc.php?edition='+serverInfo.flavor+'&version='+serverInfo.version+'&lang='+lang+'&module='+module+'&route='+layoutName;url+=app.user.get('products')?'&products='+encodeURIComponent(app.user.get('products').join(',')):'';if(layoutName=='bwc'){var action=window.location.hash.match(/#bwc.*action=(\w*)/i);if(action&&!_.isUndefined(action[1])){url+='&action='+action[1];}}
return url;},getDocumentationUrl:function(moduleName){var serverInfo=app.metadata.getServerInfo();var language=app.lang.getLanguage();var module=moduleName||app.controller.context.get('module');var products=app.user.get('products')?app.user.get('products').join(','):'';var params={edition:serverInfo.flavor,version:serverInfo.version,lang:language,};if(!_.isEmpty(products)){params.products=products;}
