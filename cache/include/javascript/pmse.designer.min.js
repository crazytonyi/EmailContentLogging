...
App.alert.dismiss('upload');w.html.style.display='inline';}});}});project.addMetadata("roles",{dataURL:'pmse_Project/CrmData/rolesList',dataRoot:"result",success:function(data){var i;if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setRoleTextField("text");f.items[emailPickerFields[i]].setRoles(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setRoleTextField("text");f.items[i].setRoles(data);break;}}}}});auxProxy=new SugarProxy({url:'pmse_Project/CrmData/related/'+PROJECT_MODULE});auxProxy.getData({cardinality:'all'},{success:function(data){var i;data=data.result;data.unshift({value:"",text:"Select..."});if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setModules(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setModules(data);break;}}}}});},submit:function(data){}};}else{items=[this._makeCriteriaField()];wHeight=185;wWidth=690;callback={loaded:function(data){root.canvas.emptyCurrentSelection();App.alert.dismiss('upload');w.html.style.display='inline';}};}}
...
ddlEmailTemplate.proxy.uid=ddlModules.value;ddlEmailTemplate.proxy.url='pmse_Project/CrmData/emailtemplates/'+ddlModules.value;aTemplate=[{'text':translate('LBL_PMSE_FORM_OPTION_SELECT'),'value':''}];ddlEmailTemplate.proxy.getData(null,{success:function(params){aTemplate=aTemplate.concat(params.result);ddlEmailTemplate.setOptions(aTemplate);App.alert.dismiss('upload');w.html.style.display='inline';}});}});project.addMetadata("roles",{dataURL:'pmse_Project/CrmData/rolesList',dataRoot:"result",success:function(data){var i;if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setRoleTextField("text");f.items[emailPickerFields[i]].setRoles(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setRoleTextField("text");f.items[i].setRoles(data);break;}}}}});auxProxy=new SugarProxy({url:'pmse_Project/CrmData/related/'+PROJECT_MODULE});auxProxy.getData({cardinality:'all'},{success:function(data){var i;data=data.result;data.unshift({value:"",text:"Select..."});if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setModules(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setModules(data);break;}}}}});},submit:function(data){}};}
break;}
...
element.validateCriteriaBoxAtoms(element,validationTools,criteria);element.validateCorrectNumberOfDateObjects(element,validationTools,criteria);};AdamEvent.prototype.validateSendMessageData=function(data,element,validationTools){var criteria=[];if(data.evn_params){criteria=JSON.parse(data.evn_params);}
validationTools.validateAtom('TEMPLATE',null,null,data.evn_criteria,element,validationTools);if(!criteria.from||!criteria.from.name||!criteria.from.id){validationTools.createWarning(element,'LBL_PMSE_ERROR_FIELD_REQUIRED','From');}
element.validateSendMessageCriteriaBoxes(element,validationTools,criteria);};AdamEvent.prototype.validateSendMessageCriteriaBoxes=function(element,validationTools,criteria){if(criteria.to&&criteria.to.length){element.validateCriteriaBoxAtoms(element,validationTools,criteria.to,true);}else{validationTools.createWarning(element,'LBL_PMSE_ERROR_FIELD_REQUIRED','To');}
if(criteria.cc&&criteria.cc.length){element.validateCriteriaBoxAtoms(element,validationTools,criteria.cc,true);}
if(criteria.bcc&&criteria.bcc.length){element.validateCriteriaBoxAtoms(element,validationTools,criteria.bcc,true);}};AdamEvent.prototype.validateCriteriaBoxAtoms=function(element,validationTools,criteria,sendEvent){var i;var atom;var module;for(i=0;i<criteria.length;i++){atom=criteria[i];module=sendEvent?atom.module:atom.expModule;if(atom.chainedRelationship){module=atom.chainedRelationship.moduleLabel;}
...
if(emailProcessUserForm){w.addPanel({title:App.lang.getModString('LBL_PMSE_FORM_LABEL_EMAIL_PROCESS_USER','pmse_Project'),panel:emailProcessUserForm});}
...
var actionDef=this.getAction(type,w);var f=new PMSE.Form({type:'action',proxy:actionDef.proxy,items:actionDef.items||[],closeContainerOnSubmit:true,footerAlign:'left',buttons:[{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_SAVE'),handler:function(){if(fieldsUpdater&&fieldsUpdater.multiplePanel){fieldsUpdater.multiplePanel.close();}
...
if(!requiredFieldIsSet){validationTools.createWarning(element,'LBL_PMSE_ERROR_FIELD_REQUIRED',field.text);}};AdamActivity.prototype.userActionById=function(userId,callback){let usersProxy=new SugarProxy();usersProxy.url='pmse_Project/CrmData/users';usersProxy.getData(null,{success:function(users){users=users.result||[];let user=users.find(function(item){return item.value===userId;});callback.call(this,{text:user?user.text:userId,value:user?user.value:userId,});}});};
