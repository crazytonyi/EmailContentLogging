...
class TenantTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @covers ::getTenant
     */
    public function testGetTenantById(): void
    {
        $srnString = 'srn:cloud:idp:eu:1234567890:tenant:12345678901';
        $expectedTenant = Tenant::fromSrn(Srn\Converter::fromString($srnString))
            ->setDisplayName($name = 'some-name')
            ->setLogo($logo = 'some-logo');

        $this->tenantRepository->method('findTenantById')->willReturn($expectedTenant);
        $this->tenantRepository
            ->expects($this->once())
            ->method('findTenantById')
            ->with($expectedTenant->getId());

        $this->session->expects($this->never())->method('get');
        $tenant = $this->function->getTenant($expectedTenant->getId());
