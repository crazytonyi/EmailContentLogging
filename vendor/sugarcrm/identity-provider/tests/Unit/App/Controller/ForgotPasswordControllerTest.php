...
class ForgotPasswordControllerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testForgotPasswordActionTestUserProviderNotFound(): void
    {
        ...
        $this->flashBag->expects($this->once())
            ...
            ->with(
                'error',
                'We could not find your account with that User Name. Please double-check and try again.'
            );
        $this->flashBag->expects($this->once())
            ->method('set')
            ->with('errorDetails', ['username' => 'username']);

        $result = $this->forgotPasswordController->forgotPasswordAction($this->application, $this->request);
    }

    /**
     * @covers ::forgotPasswordAction
     */
    public function testForgotPasswordActionDbError(): void
    {
        $this->userProvidersRepository->expects($this->once())
            ->method('findLocalByTenantAndIdentity')
            ->with('1000000001', 'username')
