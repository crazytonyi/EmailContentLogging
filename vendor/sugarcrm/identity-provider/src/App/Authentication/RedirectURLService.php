...
class RedirectURLService
{
    ...
    public function __construct(
        ...
        string $additionalAllowedDomains = ''
    ) {
    ...
    {
        $tenantDomains = [];
        if ($request->getSession() && $request->getSession()->has(TenantConfigInitializer::SESSION_KEY)) {
            $tenantDomains = $this->appService->getTenantApplicationsDomains(
        ...
        $doesDomainMatch = strpos($redirectDomain, $this->getCookiesPath($request)) ||
            in_array($redirectDomain, $tenantDomains);
