...
use Sugarcrm\IdentityProvider\App\Authentication\Consent\ConsentChecker;
...
class ConsentController
{
    ...
    public function consentInitAction(Application $app, Request $request)
    {
        ...
        try {
            ...
            if ($tenantSrn) {
                ...
                $app->getMetadataService()
                    ->getLanguagesFromSource($tenantSrn, $this->getClientUrlForMetadata($consentToken));
    ...
    /**
     * @param ConsentToken $consentToken
     * @return string
     */
    private function getClientUrlForMetadata(ConsentToken $consentToken): string
    {
        try {
            $srn = Converter::fromString($consentToken->getClientId());
        } catch (\InvalidArgumentException $e) {
            return '';
        }

        return $consentToken->getClientUrl() && Srn\Manager::isCrm($srn) ? $consentToken->getClientUrl() : '';
    }
    ...
    public function consentConfirmationAction(Application $app, Request $request)
    {
        ...
        if ($app->getUserPasswordChecker()->isPasswordExpired($userToken->getSource())) {
     ...
     */
    protected function getUserToken(): RememberMeToken
    {
        /** @var RememberMeTokenInterface $userToken */
        $userToken = $this->app->getRememberMeService()->retrieve();
